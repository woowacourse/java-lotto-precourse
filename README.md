...
- [x] 메인로직 1: 사용자로부터 로또 구입 금액 입력받기
    ...
    - ~~[x] 서브로직 1-2: 입력이 특정 길이를 초과하는지 확인~~(현재없어진 로직)
...
- [x] 메인로직 3: 사용자로부터 당첨로또번호 6개 입력받기
    ...
    - [x] 추가로직: 당첨번호입력값 6개가 모두 정수인지 확인
    - [x] 서브로직 3-2: 당첨로또번호입력값 6개가 모두 모두 1~45 인지 확인
    - [x] 서브로직 3-3: 당첨로또번호입력값 6개가 모두 중복된것이 없는지 확인
- [x] 메인로직 4: 사용자로부터 보너스번호 입력받기
    - ~~[x] 변경전로직 4-1: 보너스번호가 유효한지 확인~~
    - 변경 후 로직 4-1: 보너스번호가 정수인지 확인
    - 변경 후 로직 4-2: 보너스번호가 1~45인지 확인
    - 변경 후 로직 4-3: 보너스번호가 기존의번호와 중첩되지않는지 확인
